...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (percentage + milestoneToAssign.percentage > 100) {
            return NextResponse.json({ message: `The total weightage of ${percentage + milestoneToAssign.percentage} milestones cannot exceed 100%` }, { status: HttpStatusCode.BadRequest });
        }

        const projects = await Project.find({ finished: false, year: milestoneToAssign.year })
